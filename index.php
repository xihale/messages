...
if(!empty($_GET)) {
	...
	if ($res['type'] == 0&&strpos($res['message'],'\n')==false) header("Location: " . (preg_match("/\bhttps*://",$res['message'])===0?("http://".$res['message']):$res['message']));
	else if($res['type']==1){
		...
		<body>
		<pre><code style="font-size: 21px;"><?php echo preg_replace("/>/i","&gt;",preg_replace("/</i","&lt;",$res['message'])) ?></code></pre>
		<script defer>
		...
		header("Location:https://aword.xurl.ga/#".str_replace("\n","\\n",$res['message']));
	}
	exit(0);
}
?>
